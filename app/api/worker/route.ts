...
export async function POST(req: Request) {
  try {
    ...
    await saveClient(clientSlug, {
      ...
      // Extra intake fields stored as metadata jsonb
      metadata: {
        name: userInput.name,
        abn: userInput.abn || null,
        goal: userInput.goal || null,
        targetAudience: userInput.targetAudience || null,
        siteType: userInput.siteType || null,
        pages: Array.isArray(userInput.pages) ? userInput.pages : [],
        features: features,
        style: userInput.style || null,
        colorPrefs: userInput.colorPrefs || null,
        references: userInput.references || null,
        additionalNotes: userInput.additionalNotes || null,
        pricingMethod: userInput.pricingMethod || null,
        pricingDetails: userInput.pricingDetails || null,
        businessAddress: userInput.businessAddress || null,
        facebookPage: userInput.facebookPage || null,
        hasBooking: hasBookingFeature,
        quote: {
          package: quote.package,
          price: quote.price,
          monthlyPrice: quote.monthlyPrice,
          monthlyOngoing: quote.monthlyOngoing,
          savings: quote.savings,
          competitorPrice: quote.competitorPrice,
