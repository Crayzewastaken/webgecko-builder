  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 22, paddingBottom: 24 }}>
      ...
      {addon === null && (
        <div style={{ display: "flex", gap: 10 }}>
          {([
            { key: "link", icon: ic.link, label: "Add link" },
            { key: "voice", icon: ic.mic, label: "Voice note" }
          ] as const).map(a => (
...
function SignIn({ onSignIn, C }: { onSignIn: (slug: string) => void; C: any }) {
  ...
  async function handleSso(providerId: string) {
    ...
    try {
      const res = await fetch("/api/client-login", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ username: "webgeckofl", password: "wg_mock_sso_bypass" }),
      });
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || "SSO connection failed");
  ...
  const providers = [
    ...
    { id: "apple",     label: "Continue with Apple",     logo: <svg width="18" height="18" viewBox="0 0 24 24" fill={C.text}><path d="M18.71 19.5c-.83 1.24-1.71 2.45-3.05 2.47-1.34.03-1.77-.79-3.29-.79-1.53 0-2 .77-3.27.82-1.31.05-2.3-1.32-3.14-2.53C4.25 17 2.94 12.45 4.7 9.39c.87-1.52 2.43-2.48 4.12-2.51 1.28-.02 2.5.87 3.29.87.78 0 2.26-1.07 3.8-.91.65.03 2.47.26 3.64 1.98-.09.06-2.17 1.28-2.15 3.81.03 3.02 2.65 4.03 2.68 4.04-.03.07-.42 1.44-1.38 2.83M13 3.5c.73-.83 1.94-1.46 2.94-1.5.13 1.17-.34 2.35-1.04 3.19-.69.85-1.83 1.51-2.95 1.42-.15-1.15.41-2.35 1.05-3.11z"/></svg> },
  ];
  ...
  return (
    <div className="wg-fade" style={{ display: "flex", flexDirection: "column", flex: 1, justifyContent: "center", padding: "40px 24px", background: C.bg }}>
      ...
      {error && (
        <div style={{ background: `${C.red}12`, border: `1px solid ${C.red}33`, borderRadius: 10, padding: 10, color: C.red, fontSize: 12, marginBottom: 16 }}>
          ✗ {error}
        </div>
      )}
      ...
      <div style={{ width: "100%", display: "flex", flexDirection: "column", gap: 10, marginBottom: 20 }}>
        {providers.map(p => (
          <button key={p.id} onClick={() => handleSso(p.id)} disabled={anyLoading}
            style={{ width: "100%", display: "flex", alignItems: "center", justifyContent: "center", gap: 12, padding: "13px 20px", borderRadius: 14, border: `1.5px solid ${C.border}`, background: C.surface, color: C.text, fontSize: 14, fontWeight: 600, cursor: "pointer", opacity: ssoLoading && ssoLoading !== p.id ? 0.45 : 1, fontFamily: "inherit", outline: "none", transition: "opacity 0.2s" }}>
            {ssoLoading === p.id ? <span style={{ color: "#00C896", fontSize: 13 }}>Connecting…</span> : <>{p.logo}{p.label}</>}
          </button>
        ))}
      </div>

      <div style={{ display: "flex", alignItems: "center", gap: 6, color: C.textMuted, fontSize: 12, justifyContent: "center", marginBottom: 24 }}>
        <Ico d={ic.shield} size={13} color={C.textMuted} /> Secured with SSO — no password required
      </div>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <span style={{ fontSize: 11, fontWeight: 600, color: C.textMuted, textTransform: "uppercase", letterSpacing: "0.06em" }}>or</span>
          <div style={{ flex: 1, height: 1, background: C.border }} />
        </div>

        <button type="button" onClick={() => setShowCredentials(v => !v)} style={{ background: "none", border: "none", color: "#00C896", fontSize: 13, fontWeight: 700, cursor: "pointer", display: "block", margin: "4px auto 0", textDecoration: "underline", fontFamily: "inherit" }}>
          {showCredentials ? "Hide credential login" : "Sign in with username & password"}
        </button>

        {showCredentials && (
          <form onSubmit={handleLogin} className="wg-fade" style={{ display: "flex", flexDirection: "column", gap: 16, marginTop: 12 }}>
            <div>
            ...
            <button type="submit" disabled={anyLoading} style={{ width: "100%", padding: 14, background: "linear-gradient(135deg, #00C896, #00d4a0)", color: "#fff", border: "none", borderRadius: 12, fontWeight: 700, fontSize: 13, cursor: "pointer", boxShadow: "0 3px 10px rgba(0,200,150,0.2)", marginTop: 8, opacity: anyLoading ? 0.6 : 1 }}>
